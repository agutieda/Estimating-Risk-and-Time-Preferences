function [log_like,rho_L1,rho_CHECK] = loglike_MPL_DEU_CL(parameters,Y,menuID,algoList)
% Log-likelihood of DEU model with independent preferences and MPL data
% Modified to take risk+time tasks of CL data
...
jointTaskMenuIdx = algoList.jointTaskMenuIdx;
...
badParameter = sigma_ra<=0 || alpha_da<=0 || beta_da<=0; 
...
if badParameter == 1 % If the restrictions on the parameters are not satisfied, return a nan
    ...
    if abs(pdfCHECK-1)>0.1
        ...
        for iMenu = 1:nL
            ...
            if jointTaskMenuIdx(iMenu) == 1 % Check joint tasks only
