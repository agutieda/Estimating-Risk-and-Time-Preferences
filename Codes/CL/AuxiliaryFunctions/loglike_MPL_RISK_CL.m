function [log_like,rho_L1,rho_CHECK,pdfCHECK] = loglike_MPL_RISK_CL(parameters,Y,menuID,algoList)
% Log-likelihood of DEU model with independent preferences and MPL data
% Modified to take risk tasks of CL data only

% Objects used in estimation
nL = algoList.nL;
chosenX = algoList.chosenX;
riskTaskMenuIdx = algoList.riskTaskMenuIdx;
...
if badParameter == 1 % If the restrictions on the parameters are not satisfied, return a nan
    ...
    if abs(pdfCHECK-1)>0.1
        ...
        rho_L1 = nan(nL,1);
        for iMenu = 1:nL
            if riskTaskMenuIdx(iMenu) == 1 % Check risk tasks only
