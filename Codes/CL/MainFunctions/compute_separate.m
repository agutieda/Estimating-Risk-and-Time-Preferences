...
obj_fun = @(x) -loglike_MPL_RISK_CL(x,Y_ra,menuID_ra,algoList);

% Maximize log-likelihood function using a quasi-newton method
X0 = [mu_ra_0; sigma_ra_0];
[theta_hat,fval_ra,~,~,~,hessian_loglike] = fminunc(obj_fun,X0,options_fminunc);
...
MOM = par2mom_ra(theta_hat,algoList);
mean_ra   = MOM(1)  ;
median_ra = MOM(2)  ;
mode_ra   = MOM(3)  ;
sd_ra     = MOM(4)  ;
...
Cov_theta_hat = RobustVarCov_MPL(@loglike_MPL_RISK_CL, ...
    theta_hat,Y_ra,menuID_ra,-hessian_loglike,cluster_var,algoList,std_type);

% Use delta method to find std. error. of moments of the estimated distributions
gradient_MOM_hat = jacob_prec(@par2mom_ra,theta_hat,1e-5,algoList);
...
log_like_hat_ra = -fval_ra;

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% ESTIMATION: TIME TASKS ONLY
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Data with time tasks
Y_da = Y(timeTaskIdx);
...
% Define the objective function to minimize
obj_fun = @(x) -loglike_MPL_TIME_CL(x,Y_da,menuID_da,algoList);

% Maximize log-likelihood function using a quasi-newton method
X0 = [alpha_da_0; beta_da_0];
[theta_hat,fval_da,~,~,~,hessian_loglike] = fminunc(obj_fun,X0,options_fminunc);

% Estimated parameters
alpha_da = theta_hat(1);
beta_da  = theta_hat(2);

% Moments of the estimated distributions
MOM = par2mom_da(theta_hat,algoList);
mean_da   = MOM(1)  ;
median_da = MOM(2)  ;
mode_da   = MOM(3)  ;
sd_da     = MOM(4)  ;
...
Cov_theta_hat = RobustVarCov_MPL(@loglike_MPL_TIME_CL, ...
    theta_hat,Y_da,menuID_da,-hessian_loglike,cluster_var,algoList,std_type);

% Use delta method to find std. error. of moments of the estimated distributions
gradient_MOM_hat = jacob_prec(@par2mom_da,theta_hat,1e-5,algoList);
Cov_MOM_hat= gradient_MOM_hat*Cov_theta_hat*gradient_MOM_hat';
...
log_like_hat_da = -fval_da;

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% ESTIMATION: RISK+TIME TASKS ONLY
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Data with risk+time tasks
Y_rada = Y(jointTaskIdx);
menuID_rada = menuID(jointTaskIdx);
subjectID_rada = subjectID(jointTaskIdx);
nObs_rada = length(Y_rada);

% Define the objective function to minimize
obj_fun = @(x) -loglike_MPL_DEU_CL(x,Y_rada,menuID_rada,algoList);

% Maximize log-likelihood function using a quasi-newton method
X0 = [mu_ra_0; sigma_ra_0; alpha_da_0; beta_da_0];
[theta_hat,fval_rada,~,~,~,hessian_loglike] = fminunc(obj_fun,X0,options_fminunc);

% Estimated parameters
mu_ra_rada     = theta_hat(1);
sigma_ra_rada  = theta_hat(2);
alpha_da_rada  = theta_hat(3);
beta_da_rada   = theta_hat(4);

% Moments of the estimated distributions
MOM = par2mom(theta_hat,algoList);
mean_ra_rada   = MOM(1)  ;
median_ra_rada = MOM(2)  ;
mode_ra_rada   = MOM(3)  ;
sd_ra_rada     = MOM(4)  ;
Z_ra_rada      = MOM(5)  ;
mean_da_rada   = MOM(6)  ;
median_da_rada = MOM(7)  ;
mode_da_rada   = MOM(8)  ;
sd_da_rada     = MOM(9)  ;
Z_da_rada      = MOM(10) ;
median_df_DELTA_rada= MOM(11) ;
median_df_rr_DELTA_rada = MOM(12) ;

% Compute robust standard errors of estimated parameters
cluster_var   = subjectID_rada;
Cov_theta_hat = RobustVarCov_MPL(@loglike_MPL_DEU_CL, ...
    theta_hat,Y_rada,menuID_rada,-hessian_loglike,cluster_var,algoList,std_type);
...
% Parameters
std_mu_ra_rada    = std_theta_hat(1);
std_sigma_ra_rada = std_theta_hat(2);
std_alpha_da_rada = std_theta_hat(3);
std_beta_da_rada  = std_theta_hat(4);

% Moments
std_mean_ra_rada   = std_MOM_hat(1)  ;
std_median_ra_rada = std_MOM_hat(2)  ;
std_mode_ra_rada   = std_MOM_hat(3)  ;
std_sd_ra_rada     = std_MOM_hat(4)  ;
std_Z_ra_rada      = std_MOM_hat(5)  ;
std_mean_da_rada   = std_MOM_hat(6)  ;
std_median_da_rada = std_MOM_hat(7)  ;
std_mode_da_rada   = std_MOM_hat(8)  ;
std_sd_da_rada     = std_MOM_hat(9)  ;
std_Z_da_rada      = std_MOM_hat(10) ;
std_median_df_DELTA_rada    = std_MOM_hat(11) ;
std_median_df_rr_DELTA_rada = std_MOM_hat(12) ;

% Loglikelihood at MLE
log_like_hat_rada = -fval_rada;
